namespace RoomBuildingStarterKit.Common
{
    ...
    /// </summary>
    public class OfficeFloorCollection : MonoBehaviourPunCallbacks
    {
        /// <summary>
        ...
        float[] dataToShare = new float[4];
        private float sharedMinX;
        private float sharedMaxX;
        private float sharedMinZ;
        private float sharedMaxZ;
        ...
        // Test array
        [PunRPC]
        private void GetSharedData (float[] sharedData)
        {
            sharedMinX = sharedData[0]; 
            sharedMaxX = sharedData[1]; 
            sharedMinZ = sharedData[2]; 
            sharedMaxZ = sharedData[3];
        ...
        public OfficeFloorCollection Reset(List<FloorEntity> floorEntities, bool overrideFloorEntity = false)
        {
            ...
            {
                if(floorEntities.Min(f => f.LeftDownLocalPosition.x) != 0)
                {
                    if (dataToShare == null)
                    {
                        dataToShare[0] = floorEntities.Min(f => f.LeftDownLocalPosition.x);
                        dataToShare[1] = floorEntities.Max(f => f.LeftDownLocalPosition.x);
                        dataToShare[2] = floorEntities.Min(f => f.LeftDownLocalPosition.z);
                        dataToShare[3] = floorEntities.Max(f => f.LeftDownLocalPosition.z);


                        photonView.RPC("GetSharedData", RpcTarget.OthersBuffered, dataToShare);
                    }
                ...
                {
                    this.MinX = dataToShare[0]; 
                    this.MaxX = dataToShare[1];
                    this.MinZ = dataToShare[2];
                    this.MaxZ = dataToShare[3];
                }
