namespace RoomBuildingStarterKit.Components
{
    ...
    public abstract class ModifyFurnitureBase : BluePrintState
    {
        ...
        protected void UpdatePutFurniture()
        {
            ...
            {
                if (this is MoveFurniture)
                {
                    var groundFurniture = this.BluePrint.PayedGroundFurnitureEntities.FirstOrDefault(f => f.FurnitureType == this.editingFurniture.GetComponent<FurnitureController>().FurnitureType);
                    var wallFurniture = this.BluePrint.PayedWallFurnitureEntities.FirstOrDefault(f => f.FurnitureType == this.editingFurniture.GetComponent<FurnitureController>().FurnitureType);
                    var windowFurniture = this.BluePrint.PayedWindowFurnitureEntities.FirstOrDefault(f => f.FurnitureType == this.editingFurniture.GetComponent<FurnitureController>().FurnitureType);

                    if (groundFurniture != null)
                    {
                        this.BluePrint.PayedGroundFurnitureEntities.Remove(groundFurniture);
                        CurrencyController.inst.Increment += this.editingFurniture.GetComponent<FurnitureController>().SellPrice;
                    }
                    else if (wallFurniture != null)
                    {
                        this.BluePrint.PayedWallFurnitureEntities.Remove(wallFurniture);
                        CurrencyController.inst.Increment += this.editingFurniture.GetComponent<FurnitureController>().SellPrice;
                    }
                    else if (windowFurniture != null)
                    {
                        this.BluePrint.PayedWindowFurnitureEntities.Remove(windowFurniture);
                        CurrencyController.inst.Increment += this.editingFurniture.GetComponent<FurnitureController>().SellPrice;
                    }
                    else
                    {
                        CurrencyController.inst.Increment += this.editingFurniture.GetComponent<FurnitureController>().PurchasePrice;
                    }
                }
        ...
        private void UpdateBuildBluePrintDoor()
        {
            ...
            if (targetFloorEntity != null)
            {
                ...
                if (InputWrapper.GetKeyUp(KeyCode.Mouse0) && dir >= 0 && dir < 4 && canPutDoor)
                {
                    ...
                    this.BluePrint.BluePrintDoorFurnitureEntities.Add(new DoorFurnitureEntity(targetFloorEntity, this.BluePrint.FoundationManager.OfficeFloorCollection.GetOfficeFloorByDir(globalOfficeFloor, dir).FloorEntity, dir, FurnitureType.Common_Door, this.editingFurniture, this.BluePrint.gameObject));
