namespace RoomBuildingStarterKit.BuildSystem
{
    ...
    public class BluePrint : MonoBehaviourPunCallbacks
    {
        /// <summary>
        /// The room type.
        /// </summary>
        ...
        /// Gets or sets the blue print floor entities.
        /// </summary>
        public List<FloorEntity> BluePrintFloorEntities 
        { 
            get => this.serializableData.BluePrintFloorEntities; 
            set => this.serializableData.BluePrintFloorEntities = value; 
        }
        ...
        /// <summary>
        /// Gets or sets the payed floor entities.
        /// </summary>
        public List<FloorEntity> PayedFloorEntities { get; set; } = new List<FloorEntity>();

        /// <summary>
        /// Gets or sets the payed window furniture entities.
        /// </summary>
        public List<WindowFurnitureEntity> PayedWindowFurnitureEntities { get; set; } = new List<WindowFurnitureEntity>();

        /// <summary>
        /// Gets or sets the payed window furniture entities.
        /// </summary>
        public List<WallFurnitureEntity> PayedWallFurnitureEntities { get; set; } = new List<WallFurnitureEntity>();

        /// <summary>
        /// Gets or sets the payed window furniture entities.
        /// </summary>
        public List<GroundFurnitureEntity> PayedGroundFurnitureEntities { get; set; } = new List<GroundFurnitureEntity>();
        ...
        public void Draw(List<FloorEntity> selectedFloorEntities, bool isAdd = true, bool ignoreOutsideWindow = false, float moveRoomPositionOffsetY = 0)
        {
            ...
            foreach (var officeFloor in this.officeFloorCollection.OfficeFloors)
            {
                ...
                //var testf = PhotonNetwork.Instantiate(floorPrefab.name, floor.transform.position, Quaternion.identity);
        ...
        public void UpdateSize(bool isAddFloor)
        {
            ...
            {
                this.UpdateBluePrintFloorEntities(isAddFloor);
                return;
            }
            ...
            if (this.BluePrintData.SelectedFloorEntities.Any())
            {
                ...
                if (isAddFloor)
                {
                    this.BluePrintData.LastFrameAddFloorEntities = diffFloorEntities.ToList();
                    this.combineFloorEntities.AddRange(diffFloorEntities);
                }
                else
                {
                    this.BluePrintData.LastFrameDeleteFloorEntities = this.combineFloorEntities.Where(f => this.BluePrintData.SelectedFloorEntities.Any(e => e.Index == f.Index)).ToList();
        ...
        /// <summary>
        /// Updates blue print floor entities.
        /// </summary>
        private void UpdateBluePrintFloorEntities(bool isAddFloor)
        {
        ...
        /// <summary>
        /// Calculates build room cost.
        /// </summary>
        /// <param name="isAddFloor">Is add floor or not.</param>
        private void CalculateCost(bool isAddFloor)
        {
            if (isAddFloor && this.BluePrintData.LastFrameAddFloorEntities != null)
            {
                if (CurrencyController.inst != null)
                {
                    CurrencyController.inst.Increment -= this.BluePrintData.LastFrameAddFloorEntities.Count * this.RealRoom.PurchasePricePerGrid;
                }
            }
            else if (this.BluePrintData.LastFrameDeleteFloorEntities != null)
            {
                var sellGridCount = this.PayedFloorEntities.RemoveAll(f => this.BluePrintData.LastFrameDeleteFloorEntities.Any(ff => ff == f));

                if (CurrencyController.inst != null)
                {
                    CurrencyController.inst.Increment += sellGridCount * this.RealRoom.SellPricePerGrid;
                    CurrencyController.inst.Increment += (this.BluePrintData.LastFrameDeleteFloorEntities.Count - sellGridCount) * this.RealRoom.PurchasePricePerGrid;
                }
            }

            this.BluePrintData.LastFrameAddFloorEntities = null;
            this.BluePrintData.LastFrameDeleteFloorEntities = null;
        }
        ...
        {
            CurrencyController.inst?.Add(this.CalculateRoomSellPrice());
        ...
        public void CopyFrom(string data)
        {
            ...
            if (CurrencyController.inst != null)
            {
                CurrencyController.inst.Increment = -this.CalculateRoomPurchasePrice();
            }
        }
        ...
        public void CancelBluePrintMode()
        {
            ...
            {
                // 0. Clear cost.
                CurrencyController.inst.Increment = 0;
            }
            ...
            // Continue NPCs when blue print state machine stop.
            TimeController.inst?.LeaveBluePrintMode();
        ...
        {
            this.CleanNPCInteractingWithFurnitures();
            ...
            // Init payed floor entities and furniture entities every time enter blue print mode.
            this.PayedFloorEntities = this.BluePrintFloorEntities.ToList();
            this.PayedWindowFurnitureEntities = this.BluePrintWindowFurnitureEntities.ToList();
            this.PayedWallFurnitureEntities = this.BluePrintWallFurnitureEntities.ToList();
            this.PayedGroundFurnitureEntities = this.BluePrintFurnitureEntities.ToList();
            ...
            // Pause NPCs when blue print state machine run.
            TimeController.inst?.EnterBluePrintMode();
        ...
        public void OnBuildRoomCompleteButtonClicked(Action callback)
        {
            ...
            if (CurrencyController.inst != null && CurrencyController.inst.CanAfford() == false)
            {
                ToastMessage.inst.EnqueueMessage(UIText.MONEY_NOT_ENOUGH);
                return;
            }

            //this.OnBuildRoomCompleted();
            photonView.RPC("OnBuildRoomCompleted", RpcTarget.AllBuffered);
            callback();
        }
        ...
        /// <summary>
        /// Calculates the room sell price includes grids and furnitures.
        /// </summary>
        /// <returns>The room sell price.</returns>
        public double CalculateRoomSellPrice()
        {
            return this.BluePrintFloorEntities.Count * this.RealRoom.SellPricePerGrid
                + this.BluePrintWindowFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().SellPrice)
                + this.BluePrintWallFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().SellPrice)
                + this.BluePrintFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().SellPrice);
        }

        /// <summary>
        /// Calculates the room purchase price includes grids and furnitures.
        /// </summary>
        /// <returns>The room purchase price.</returns>
        public double CalculateRoomPurchasePrice()
        {
            return this.BluePrintFloorEntities.Count * this.RealRoom.PurchasePricePerGrid
                + this.BluePrintWindowFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().PurchasePrice)
                + this.BluePrintWallFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().PurchasePrice)
                + this.BluePrintFurnitureEntities.Sum(f => f.Furniture.GetComponent<FurnitureController>().PurchasePrice);
        }

        /// <summary>
        /// When a room enter blue print mode, need to cancel interact state of NPCs with furnitures inside this room.
        /// </summary>
        private void CleanNPCInteractingWithFurnitures()
        {
            this.BluePrintFurnitureEntities.ForEach(f => f.Furniture.GetComponent<FurnitureController>().InteractTriggers.ForEach(t =>
            {
                if (t.OccupiedNPC != null)
                {
                    t.OccupiedNPC.GetComponents<PlayMakerFSM>().First(fsm => fsm.FsmName == "NPC_Action").SendEvent("EnterBluePrintMode");
                }
            }));
        }
        ...
        private void Assign(BluePrintSerializableData bluePrint, bool isCopy = false)
        {
            ...
            if (bluePrint.BluePrintWindowFurnitureEntities.Any())
            {
                this.BluePrintWindowFurnitureEntities = bluePrint.BluePrintWindowFurnitureEntities.Select(w =>
                {
                    ...
                    return new WindowFurnitureEntity(floorEntity, w.Direction, w.FurnitureType, window, this.gameObject);
                }).ToList();
            }
            ...
            if (bluePrint.BluePrintWallFurnitureEntities.Any())
            {
                this.BluePrintWallFurnitureEntities = bluePrint.BluePrintWallFurnitureEntities.Select(w =>
                {
                    ...
                    return new WallFurnitureEntity(floorEntity, floorEntities, w.Direction, w.FurnitureType, furniture, this.gameObject, isCopy ? new FurnitureCustomPersistentProperties(w.CustomProperties) : w.CustomProperties);
                }).ToList();
            }
            ...
            if (bluePrint.BluePrintFurnitureEntities.Any())
            {
                this.BluePrintFurnitureEntities = bluePrint.BluePrintFurnitureEntities.Select(f =>
                {
                    ...
                    FurnitureHelper.ChangeFurnitureLayer(furniture, LayerMask.NameToLayer("Selectable"));
                    Debug.Log($"{isCopy} {f.CustomProperties}");
                    return new GroundFurnitureEntity(floorEntity, floorEntities, f.Direction, f.FurnitureType, furniture, this.gameObject, isCopy ? new FurnitureCustomPersistentProperties(f.CustomProperties) : f.CustomProperties);
                }).ToList();
            }
        }
